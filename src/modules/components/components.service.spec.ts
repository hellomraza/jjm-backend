...
describe('ComponentsService', () => {
  ...
  it('approveComponent stores the photo approval timestamp', async () => {
    ...
    const componentMapping = {
      ...
      approved_photo_id: 'p1',
      work_item_id: 'w1',
      quantity: 10,
      progress: 4,
      workItem: { district_id: 7 },
    };
    const selectedPhoto = {
      id: 'p1',
      component_id: 'wc1',
    ...
    const manager = {
      findOne: jest
        .fn()
        .mockResolvedValueOnce(districtOfficer)
        .mockResolvedValueOnce(componentMapping)
        .mockResolvedValueOnce(selectedPhoto),
      save: jest.fn(async (_entity, value) => value),
      count: jest.fn().mockResolvedValueOnce(12).mockResolvedValueOnce(6),
      update: jest.fn(),
    };
    ...
    const result = await service.approveComponent('wc1', 'do1');
